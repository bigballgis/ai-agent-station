...
> 当前用途：记录安全基线和持续验证要求。本文不把历史完成状态当作事实；每项安全能力必须能被代码、配置、测试或运行结果证明。

## 0. 安全执行原则

- 任何 AI 任务不得降低认证、授权、租户隔离、审计和日志脱敏要求。
- 新增工具、MCP Server、HTTP 调用和工作流节点必须具备超时、错误分类、审计与敏感信息保护。
- 生产密钥必须来自环境变量、K8s Secret、Vault 或云密钥服务，禁止写入仓库。
- 发现文档与代码不一致时，先标注未验证或修正文档，不允许按理想状态继续开发。
