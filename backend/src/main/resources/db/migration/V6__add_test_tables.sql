...
CREATE TABLE IF NOT EXISTS agent_test_cases (
  ...
  CONSTRAINT uk_test_code UNIQUE (test_code, tenant_id)
);
...
CREATE TRIGGER update_agent_test_cases_updated_at BEFORE UPDATE ON agent_test_cases
...
CREATE INDEX IF NOT EXISTS idx_agent_test_executions_tenant_id ON agent_test_executions(tenant_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_executions_agent_id ON agent_test_executions(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_executions_test_case_id ON agent_test_executions(test_case_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_executions_status ON agent_test_executions(status);
...
CREATE INDEX IF NOT EXISTS idx_agent_test_results_execution_id ON agent_test_results(execution_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_results_tenant_id ON agent_test_results(tenant_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_results_agent_id ON agent_test_results(agent_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_results_test_case_id ON agent_test_results(test_case_id);
CREATE INDEX IF NOT EXISTS idx_agent_test_results_status ON agent_test_results(status);

-- 添加外键约束
ALTER TABLE agent_test_cases ADD CONSTRAINT fk_agent_test_cases_agent FOREIGN KEY (agent_id) REFERENCES agent(id);
ALTER TABLE agent_test_executions ADD CONSTRAINT fk_agent_test_executions_agent FOREIGN KEY (agent_id) REFERENCES agent(id);
ALTER TABLE agent_test_executions ADD CONSTRAINT fk_agent_test_executions_test_case FOREIGN KEY (test_case_id) REFERENCES agent_test_cases(id);
ALTER TABLE agent_test_results ADD CONSTRAINT fk_agent_test_results_execution FOREIGN KEY (execution_id) REFERENCES agent_test_executions(id);
ALTER TABLE agent_test_results ADD CONSTRAINT fk_agent_test_results_agent FOREIGN KEY (agent_id) REFERENCES agent(id);
ALTER TABLE agent_test_results ADD CONSTRAINT fk_agent_test_results_test_case FOREIGN KEY (test_case_id) REFERENCES agent_test_cases(id);

-- 更新创建租户schema的函数，添加测试相关表
...
('test.data.isolation.enabled', 'true', 'boolean', '测试数据隔离是否启用'),
('test.data.retention.days', '30', 'number', '测试数据保留天数'),
('test.execution.timeout', '60', 'number', '测试执行超时时间(秒)'),
('test.concurrent.limit', '10', 'number', '测试并发执行限制');

-- 添加测试相关的权限
INSERT INTO permission (permission_name, permission_code, description) VALUES
('测试管理', 'TEST_MANAGEMENT', '管理测试用例和执行'),
('测试执行', 'TEST_EXECUTION', '执行测试用例'),
('测试结果查看', 'TEST_RESULT_VIEW', '查看测试结果');
...
SELECT (SELECT id FROM super_admin_role), id FROM permission WHERE permission_code IN ('TEST_MANAGEMENT', 'TEST_EXECUTION', 'TEST_RESULT_VIEW')
ON CONFLICT DO NOTHING;

-- 租户管理员拥有测试管理权限
INSERT INTO role_permission (role_id, permission_id)
SELECT (SELECT id FROM tenant_admin_role), id FROM permission WHERE permission_code IN ('TEST_MANAGEMENT', 'TEST_EXECUTION', 'TEST_RESULT_VIEW')
ON CONFLICT DO NOTHING;

-- Agent设计者拥有测试执行和查看权限
INSERT INTO role_permission (role_id, permission_id)
SELECT (SELECT id FROM agent_designer_role), id FROM permission WHERE permission_code IN ('TEST_EXECUTION', 'TEST_RESULT_VIEW')
ON CONFLICT DO NOTHING;

-- 审批者拥有测试结果查看权限
INSERT INTO role_permission (role_id, permission_id)
SELECT (SELECT id FROM approver_role), id FROM permission WHERE permission_code = 'TEST_RESULT_VIEW'
ON CONFLICT DO NOTHING;

-- 观察者拥有测试结果查看权限
INSERT INTO role_permission (role_id, permission_id)
SELECT (SELECT id FROM viewer_role), id FROM permission WHERE permission_code = 'TEST_RESULT_VIEW'
ON CONFLICT DO NOTHING;