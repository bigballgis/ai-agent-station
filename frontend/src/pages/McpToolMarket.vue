      ...
      >
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            :aria-label="t('mcpTool.testConnection')"
...
async function handleTestConnection(tool: McpTool) {
  ...
  try {
    ...
    if (status === 'HEALTHY') {
      message.success(t('mcpTool.connectionHealthy'))
    } else if (status === 'DEGRADED') {
      message.warning(t('mcpTool.connectionDegraded'))
    } else {
      message.error(t('mcpTool.connectionFailed'))
    }
  } catch (e: any) {
    logger.error('Test connection failed:', e)
    message.error(t('mcpTool.connectionTestFailed', { message: e?.message || 'Unknown error' }))
  } finally {
...
function connectTool() {
  ...
  refreshTools().then(() => {
    ...
    logger.error('Failed to refresh tools:', e)
